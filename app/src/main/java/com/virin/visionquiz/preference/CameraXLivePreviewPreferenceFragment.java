...
public class CameraXLivePreviewPreferenceFragment extends LivePreviewPreferenceFragment {
    ...
    private void setUpCameraXTargetAnalysisSizePreference(
        ...
        if (pref.getEntry() == null) {
            String defaultValue =
                    selectClosestSize(
                            Arrays.asList(entries), PreferenceUtils.getDefaultCameraXTargetResolution());
            pref.setValue(defaultValue);
            pref.setSummary(defaultValue);
            PreferenceUtils.saveString(getActivity(), previewSizePrefKeyId, defaultValue);
        } else {
    ...
    private static String selectClosestSize(List<String> supportedSizes, Size targetSize) {
        String bestSize = null;
        int bestDiff = Integer.MAX_VALUE;
        for (String sizeString : supportedSizes) {
            Size parsedSize = Size.parseSize(sizeString);
            int diff = abs(parsedSize.getWidth() - targetSize.getWidth())
                    + abs(parsedSize.getHeight() - targetSize.getHeight());
            if (diff < bestDiff) {
                bestDiff = diff;
                bestSize = sizeString;
            }
        }
        return bestSize != null ? bestSize : PreferenceUtils.getDefaultCameraXTargetResolutionString();
    }
}
