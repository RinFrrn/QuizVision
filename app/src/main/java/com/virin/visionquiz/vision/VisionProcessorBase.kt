...
import com.virin.visionquiz.BitmapUtils
import com.virin.visionquiz.vision.graphic.CameraImageGraphic
import com.virin.visionquiz.FrameMetadata
import com.virin.visionquiz.vision.graphic.GraphicOverlay
import com.virin.visionquiz.ScopedExecutor
...
abstract class VisionProcessorBase<T>(context: Context) : VisionImageProcessor {
  ...
  private val executor = ScopedExecutor(TaskExecutors.MAIN_THREAD)//Executors.newFixedThreadPool(1)
  ...
  private var inferenceInfoListener: ((InferenceInfo?) -> Unit)? = null

  fun setInferenceInfoListener(listener: ((InferenceInfo?) -> Unit)?) {
    inferenceInfoListener = listener
  }
  ...
  override fun processBitmap(bitmap: Bitmap?, graphicOverlay: GraphicOverlay) {
    processBitmap(bitmap, graphicOverlay, null)
  }
  ...
  private fun processImage(
    ...
    if (isMlImageEnabled(graphicOverlay.context)) {
      ...
      requestDetectInImage(mlImage, graphicOverlay, bitmap, /* shouldShowFps= */ true, frameStartMs)
        .addOnCompleteListener(executor) { processLatestImage(graphicOverlay) }
    ...
    return task
      .addOnSuccessListener(
        ...
        OnSuccessListener { results: T ->
          ...
          inferenceInfoListener?.invoke(
            if (PreferenceUtils.shouldHideDetectionInfo(graphicOverlay.context)) {
              null
            } else {
              InferenceInfo(
                imageWidth = graphicOverlay.imageWidth,
                imageHeight = graphicOverlay.imageHeight,
                frameLatencyMs = currentFrameLatencyMs,
                detectorLatencyMs = currentDetectorLatencyMs,
                framesPerSecond = if (shouldShowFps) framesPerSecond else null
              )
            }
            )
